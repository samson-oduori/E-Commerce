...
$errors = array();
	...
	$db = mysqli_connect('localhost', 'root', '', 'acaciaadmin');
...
if($num == 1){
	array_push($errors, "The phone number already exists");
		...
		if(count($errors) == 0){
			...
			$sql = "INSERT INTO hostel_reg (phone, password) VALUES ('$username', '$password')";
			mysqli_query($db, $sql);
	...
	if (isset($_POST['login'])){
		...
		if (count($errors) == 0 ){
			...
			$query= "SELECT * FROM hostel_reg WHERE phone = '$username' AND password = '$password'";
